 ...
 *   variables in VARS order; inside each variable, time steps outer,
 *   levels inner; each record holds nx*ny big-endian float32 values with
 *   x varying fastest. OPTIONS sequential wraps every record in 4-byte
 *   Fortran markers. OPTIONS byteswapped swaps each 4-byte word.
 *
...
/* Read one (var, t, z) slice into out[ny*nx] doubles, x fastest.
 * Indices are 0-based. Returns 0 on success, -1 with *err_out set. */
