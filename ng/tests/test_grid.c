...
int main(int argc, char **argv) {
    ...
    /* Byte-exact reads of every slice of both variables. */
    for (t = 0; t < 2; t++) {
        /* var 0 tsfc (surface): z must be 0. */
        CHECK(ng_grid_read_slice(g, 0, t, 0, out, &err) == 0,
        ...
        bad = 0;
        CHECK(ng_grid_read_slice(g, 0, t, 1, out, &err) != 0 && err,
              "tsfc z=1 rejected (%s)", err ? err : "no message");

        /* var 1 hgt (2 levels). */
        for (z = 0; z < 2; z++) {
            CHECK(ng_grid_read_slice(g, 1, t, z, out, &err) == 0,
    ...
    /* Index validation. */
    CHECK(ng_grid_read_slice(g, -1, 0, 0, out, &err) != 0 && err,
          "negative var rejected");
    CHECK(ng_grid_read_slice(g, 2, 0, 0, out, &err) != 0 && err,
          "var past end rejected");
    CHECK(ng_grid_read_slice(g, 0, 2, 0, out, &err) != 0 && err,
          "time past end rejected");
    CHECK(ng_grid_read_slice(g, 0, 0, 0, NULL, &err) != 0 && err,
          ...
          "write sequential/LE fixtures");
    {
        ...
        if (c4) {
            ...
            if (g4) {
                for (t = 0; t < 2; t++) {
                    CHECK(ng_grid_read_slice(g4, 0, t, 0, out, &err) == 0,
